...
const isAllowedExpiration = (value: unknown): value is number =>
  typeof value === "number" && Number.isInteger(value) && value >= 0 && value <= 999999999;
...
export async function POST(request: Request) {
  ...
  if (!isAllowedExpiration(body.expirationDays)) {
    return NextResponse.json({ error: `expirationDays must be 0 (permanent) or a positive integer`, code: 'invalid_expiration' }, { status: 400 });
  }
