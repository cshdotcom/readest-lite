  ...
  return (
    ...
    >
        ...
        <ul className='ms-0 flex flex-col ps-0 before:hidden'>
          ...
          {!isPinEnabled && (
            <MenuItem
              ...
              tooltip={
                appService?.isMobileApp
                  ? _('Require a PIN (and biometrics, if available) to open Readest Lite')
                  : _('Require a 4-digit PIN to open Readest Lite')
              }
      ...
      {user && userProfilePlan === 'free' && (
        <MenuItem label={_('Upgrade to Readest Lite Premium')} onClick={handleUpgrade} />
      )}
      {isWebAppPlatform() && <MenuItem label={_('Download Readest Lite')} onClick={downloadReadest} />}
      <MenuItem label={_('About Readest Lite')} onClick={showAboutReadest} />
    </Menu>
  );
};
